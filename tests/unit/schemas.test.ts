...
describe('Schemas Zod', () => {
  ...
  describe('BankAccountCreateSchema', () => {
    ...
    it('rejeita nome vazio', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Nome da conta é obrigatório');
      }
    });
  });
  ...
  describe('CreditCardCreateSchema', () => {
    ...
    it('rejeita nome vazio', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Nome do cartão é obrigatório');
      }
    });
  });
  ...
  describe('TransactionCreateSchema', () => {
    ...
    it('rejeita configuração inválida (conta E cartão)', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Configuração de transação inválida');
      }
    });
  });
  ...
  describe('TransferCreateSchema', () => {
    ...
    it('rejeita transferência para a mesma conta', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Não é possível transferir para a mesma conta');
      }
    });
