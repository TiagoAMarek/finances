...
import { TrendingUp, TrendingDown, Calendar, BarChart3, Activity, Zap } from "lucide-react";
...
export function ComparativePerformanceCards({
  ...
  const last30DaysTransactions = transactions.filter((t) => {
    const transactionDate = new Date(t.date);
    return transactionDate >= thirtyDaysAgo && transactionDate <= now && t.type === 'expense';
  });

  const last30DaysExpenses = last30DaysTransactions.reduce((sum, t) => sum + parseFloat(t.amount), 0);
  ...
  const lastThreeMonthsTransactions = transactions.filter((t) => {
    const transactionDate = new Date(t.date);
    return transactionDate >= threeMonthsAgo && transactionDate <= now && t.type === 'expense';
  });

  const threeMonthsAverage = lastThreeMonthsTransactions.length > 0 
    ? lastThreeMonthsTransactions.reduce((sum, t) => sum + parseFloat(t.amount), 0) / 3
    : 0;

  const trendVsAverage = threeMonthsAverage > 0 
  ...
  const dayOfMonth = targetMonth === currentDate.getMonth() && targetYear === currentDate.getFullYear() 
    ? currentDate.getDate() 
    : daysInMonth;
  
  const projectedMonthExpenses = dayOfMonth > 0 ? (currentMonthExpenses / dayOfMonth) * daysInMonth : currentMonthExpenses;
  const spendingVelocity = previousMonthExpenses > 0 
    ? ((projectedMonthExpenses - previousMonthExpenses) / previousMonthExpenses) * 100 
    : 0;
  ...
  const comparativeCards = [
    ...
    },
    {
      ...
      color: spendingVelocity >= 10 ? "text-red-500" : spendingVelocity >= 0 ? "text-yellow-500" : "text-green-500",
      bgColor: spendingVelocity >= 10 ? "bg-red-500/10" : spendingVelocity >= 0 ? "bg-yellow-500/10" : "bg-green-500/10"
    },
    {
      title: "Padrão Mensal",
