...
export function FinancialInsights({
  ...
  const generateInsights = () => {
    ...
    if (previousMonthExpenses > 0) {
      const expenseChange = ((monthlyExpenses - previousMonthExpenses) / previousMonthExpenses) * 100;
    ...
    if (previousMonthIncomes > 0) {
      const incomeChange = ((monthlyIncomes - previousMonthIncomes) / previousMonthIncomes) * 100;
      if (Math.abs(incomeChange) > 10) {
        insights.push({
          type: incomeChange > 0 ? 'success' : 'warning',
          title: incomeChange > 0 ? 'Receitas aumentaram' : 'Receitas diminuíram',
    ...
    if (monthlyTransactions.length > 0) {
      ...
      const expensiveDay = Object.entries(dailyTotals)
        .sort(([,a], [,b]) => b - a)[0];
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        {insights.map((insight, index) => {
          ...
          return (
            <Alert key={index} variant={insight.type as "default" | "destructive"}>
