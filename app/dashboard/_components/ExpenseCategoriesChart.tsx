...
export function ExpenseCategoriesChart({ 
  ...
  const generateCategoryData = () => {
    ...
    if (selectedAccountId !== null && selectedAccountId !== undefined) {
      monthlyExpenses = monthlyExpenses.filter(t => t.accountId === selectedAccountId);
    }
    if (selectedCreditCardId !== null && selectedCreditCardId !== undefined) {
      monthlyExpenses = monthlyExpenses.filter(t => t.creditCardId === selectedCreditCardId);
    }
  ...
  if (categoryData.length > 5) {
    const othersTotal = categoryData.slice(5).reduce((sum, item) => sum + item.amount, 0);
